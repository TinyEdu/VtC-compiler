...
class Bin : public Expr {
 public:
  void process(Expr* expr) override { expr->process(this); }

  void process(Bin* expr) override { std::cout << "Bin in Bin" << std::endl; }

  void process(Grp* expr) override { std::cout << "Grp in Bin" << std::endl; }
};
...
class Grp : public Expr {
 public:
  ...
  void process(Bin* expr) override { std::cout << "Bin in Grp" << std::endl; }

  void process(Grp* expr) override { std::cout << "Grp in Grp" << std::endl; }
};
...
Functions should return Literal * and at end call an outside function
    ...
    Review(TokenType, l, r)
        ...
        Template Static Literal
        Review(T1 a, T2 b, TokenType type) Switch type : Case(plus);
return reviewer::add(a, b);

Can I write a template function like this Template Static Literal
Review(T1 a, T2 b, TokenType type) Switch type : Case(plus);
return reviewer::add(a, b);

And then declare all possible combinations of reviewer::add() Like
    this add(Bin, Grp) add(Grp, Bin) Etc.
...
auto divide(const T1& a, const T2& b)
    -> std::enable_if_t<!std::is_same_v<T1, Bin> || !std::is_same_v<T2, Grp>,
                        decltype(a / b)> {
  return a / b;
}
...
auto divide(const T1& a, const T2& b)
    -> std::enable_if_t<std::is_same_v<T1, Bin> && std::is_same_v<T2, Grp>,
                        void> {
  std::cerr << "Error: Division between Bin and Grp is not supported!"
            << std::endl;
