...
std::map<std::string, std::string> typeMap = {{"LiteralInt", "int"},
                                              {"LiteralFloat", "float"},
...
int main() {
  ...
  for (auto& type : types) {
    ...
    cppfile
        << "Expression* " + type +
               "::process(LiteralInt* expr, Token token) {\n"
               "  return OperationsDispatcher::dispatch(expr, this, token);\n"
               "};\n\n";
    cppfile
        << "Expression* " + type +
               "::process(LiteralFloat* expr, Token token) {\n"
               "  return OperationsDispatcher::dispatch(expr, this, token);\n"
               "};\n\n";
    cppfile
        << "Expression* " + type +
               "::process(LiteralString* expr, Token token) {\n"
               "  return OperationsDispatcher::dispatch(expr, this, token);\n"
               "};\n\n";
    cppfile
        << "Expression* " + type +
               "::process(LiteralBool* expr, Token token) {\n"
               "  return OperationsDispatcher::dispatch(expr, this, token);\n"
               "};\n\n";
    cppfile
        << "Expression* " + type +
               "::process(LiteralDouble* expr, Token token) {\n"
               "  return OperationsDispatcher::dispatch(expr, this, token);\n"
               "};\n\n";
    cppfile.close();
  }
