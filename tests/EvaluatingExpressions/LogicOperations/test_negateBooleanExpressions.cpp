...
TEST(InterpreterTest, DoubleNegation) {
  ...
  Expression* expr1 =
      new Unary(Token(TokenType::BANG),
                new Unary(Token(TokenType::BANG), new Literal(true)));
  Expression* expr2 =
      new Unary(Token(TokenType::BANG),
                new Unary(Token(TokenType::BANG), new Literal(false)));
...
TEST(InterpreterTest, NegateWithArithmetic) {
  ...
  Expression* expr1 =
      new Unary(Token(TokenType::BANG), new Literal(1 + 1 == 2));
  // Negating a false condition (2 + 2 == 5)
  Expression* expr2 =
      new Unary(Token(TokenType::BANG), new Literal(2 + 2 == 5));
